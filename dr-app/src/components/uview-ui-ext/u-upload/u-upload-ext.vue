<template>
	<view class="u-upload" v-if="!disabled">
		...
		<uni-popup ref="popupDialog" type="dialog">
			<uni-popup-dialog :before-close="true" @close="popupClose" @confirm="popupConfirm" content="您确定要删除此项吗？" title="提示" type="error"></uni-popup-dialog>
		</uni-popup>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			doPreviewImage(url, index) {
				...
				let getOriginUrl = (url) => {
					return url.replace('/thumbnail', '');
				};
				const images = this.lists.map(item => getOriginUrl(item.url) || item.path);
				let originUrl = getOriginUrl(url);
				console.log('原图', originUrl);
				uni.previewImage({
					urls: images,
					current: originUrl,
					success: () => {
