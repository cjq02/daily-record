	...
	export default {
		...
		data() {
			return {
				...
				minutes: _.range(0, 60, this.minuteStep),
				seconds: _.range(0, 60, this.minuteStep),
			};
		},
		methods: {
			//设置值
			...
			onTimeChange(e) {
				...
				let minute = this.minutes[e.detail.value[1]];
				let second = this.seconds[e.detail.value[2] || 0];
