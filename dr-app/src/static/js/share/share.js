function shareFn(shareInfo, drawList, callback) {
	...
	let alphaButton = new plus.nativeObj.View("alphaButton", { //先创建取消按钮
		bottom: '0px',
		left: '0px',
		height: '44px',
		width: '100%',
	...
	alphaButton.addEventListener("click", function() { //处理取消按钮点击
		alphaBg.hide();
		shareMenu.hide();
		alphaButton.hide();
	})
	let shareMenu = new plus.nativeObj.View("shareMenu", { //创建底部图标菜单
		bottom: '44px',
		left: '0px',
		height: Math.ceil(drawList.length / colNumber) * itemHeight + marginTop + 'px',
		width: '100%',
		backgroundColor: 'rgb(255,255,255)'
	});
	//绘制底部按钮的内容
