<template>
	<view class="u-upload" v-if="!disabled">
		<view :key="index" :style="{
			...
			}" class="u-list-item u-preview-wrap" v-for="(item, index) in lists" v-if="showUploadList">
			<view :style="{
					background: delBgColor
				}" @tap.stop="deleteItem(index)" class="u-delete-icon" v-if="deletable">
				<u-icon class="u-icon" :name="delIcon" size="20" :color="delColor"></u-icon>
			</view>
			<u-line-progress :percent="item.progress" :show-percent="false" class="u-progress" height="16"
			 v-if="showProgress && item.progress > 0 && !item.error"></u-line-progress>
			<view @tap.stop="retry(index)" v-if="item.error" class="u-error-btn">点击重试</view>
			<image :mode="imageMode" :src="item.url || item.path" @tap.stop="doPreviewImage(item.url || item.path, index)" class="u-preview-image"
			 v-if="!item.isImage"></image>
		</view>
		...
		<view style="display: inline-block;" @tap="selectFile" v-if="maxCount > lists.length">
			...
			<view :style="{
					width: $u.addUnit(width),
					height: $u.addUnit(height)
				}" class="u-list-item u-add-wrap" hover-class="u-add-wrap__hover" hover-stay-time="150" v-if="!customBtn">
